...
public class EnhancedCustomerAI
{
    ...
    /// </summary>
    public EnhancedPurchaseDecision MakeEnhancedPurchaseDecision(Item item, decimal price,
        ShopInteractionContext context)
    {
    ...
    private Models.PurchaseDecision ApplyEnhancedLogic(Models.PurchaseDecision baseDecision, DecisionFactors factors)
    {
        ...
        if (baseDecision == Models.PurchaseDecision.Considering)
        {
            ...
            {
                return factors.NegotiationLikelihood > 0.5f
                    ? Models.PurchaseDecision.WantsToNegotiate
                    : Models.PurchaseDecision.Buying;
            }
    ...
    /// </summary>
    private DecisionReasoning GenerateDecisionReasoning(Item item, decimal price, DecisionFactors factors,
        Models.PurchaseDecision decision)
    {
    ...
    private string DetermineSuggestedPlayerAction(Models.PurchaseDecision decision, DecisionFactors factors)
    {
        ...
        {
            Models.PurchaseDecision.Buying when factors.OverallConfidence > 0.9f =>
                "Complete the sale - customer is ready to buy!",
            Models.PurchaseDecision.Buying => "Finalize the sale, but be prepared for last-minute questions",
            Models.PurchaseDecision.WantsToNegotiate =>
                $"Customer wants to negotiate - they might accept {factors.NegotiationLikelihood * 100:F0}% of asking price",
            Models.PurchaseDecision.Considering when factors.InteractionSatisfaction < 0.5f =>
                "Try engaging the customer with helpful information",
            Models.PurchaseDecision.Considering when factors.QualityScore < 0.5f =>
                "Show the customer higher quality alternatives",
            Models.PurchaseDecision.Considering when factors.PriceScore < 0.5f =>
                "Consider offering a discount or showing cheaper alternatives",
            Models.PurchaseDecision.NotBuying when factors.AlternativeInterest > 0.6f =>
                "Show different items that might interest them",
            Models.PurchaseDecision.NotBuying when factors.InteractionSatisfaction > 0.6f =>
                "Customer likes you but not the item - try alternatives",
    ...
    private float AnalyzePriceAcceptability(Item item, decimal price) =>
        _customer.BudgetRange.CanAfford(price)
            ? Math.Max(0f, 1f - (float)price / _customer.BudgetRange.MaxSpendingPower)
            : 0f;
