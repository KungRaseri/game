...
public class ShopManager
{
    ...
    public decimal CalculateSuggestedPrice(Item item, float profitMargin = 0.5f)
    {
        ...
        GameLogger.Debug(
            $"Calculated market price for {item.Name}: {marketPrice} gold (base: {baseValue}, market-adjusted from: {basePriceWithMargin})");
    ...
    */
    ...
    public SaleTransaction? ProcessSale(int slotId, string customerId, CustomerSatisfaction satisfaction)
    {
        ...
        GameLogger.Info(
            $"Processed sale: {item.Name} sold for {salePrice} gold to customer {customerId} (profit: {profit})");
    ...
    public ShopPerformanceMetrics GetPerformanceMetrics()
    {
        ...
        var metrics = new ShopPerformanceMetrics
        {
            ...
            AverageTransactionValue = _transactionHistory.Count > 0 ? _transactionHistory.Average(t => t.SalePrice) : 0,
            TotalProfit = _transactionHistory.Sum(t => t.SalePrice - t.EstimatedCost),
            AverageProfitMargin = _transactionHistory.Count > 0 ? _transactionHistory.Average(t => t.ProfitMargin) : 0,
