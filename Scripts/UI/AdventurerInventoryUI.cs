...
public partial class AdventurerInventoryUI : Panel
{
    ...
    {
        GameLogger.Info(
            $"AdventurerInventoryUI: UpdateInventoryDisplay called. InventoryManager={_inventoryManager}, InventoryList={_inventoryList}");
        ...
        */
        ...
        GameLogger.Info(
            $"AdventurerInventoryUI: Display update completed. InventoryList now has {_inventoryList.GetChildCount()} children");

        // Debug container sizes to check if layout is working
        if (_scrollContainer != null)
        {
            GameLogger.Debug(
                $"AdventurerInventoryUI: ScrollContainer size: {_scrollContainer.Size}, visible: {_scrollContainer.Visible}");
        }

        GameLogger.Debug(
            $"AdventurerInventoryUI: InventoryList size: {_inventoryList.Size}, visible: {_inventoryList.Visible}");
    ...
    private void CreateMaterialStackUI(MaterialStack materialStack)
    {
        ...
        {
            GameLogger.Debug(
                $"AdventurerInventoryUI: Instantiating MaterialStackScene for {materialStack.Material.Name}");
            ...
            GameLogger.Debug(
                $"AdventurerInventoryUI: MaterialStackUI created - Visible: {materialStackInstance.Visible}, Size: {materialStackInstance.Size}, Position: {materialStackInstance.Position}");
            GameLogger.Debug(
                $"AdventurerInventoryUI: InventoryList size: {_inventoryList.Size}, children count: {_inventoryList.GetChildCount()}");

            GameLogger.Info(
                $"AdventurerInventoryUI: Successfully created MaterialStackUI for {materialStack.Material.Name} x{materialStack.Quantity}");
        }
        catch (Exception ex)
        {
            GameLogger.Error(ex,
                $"AdventurerInventoryUI: Failed to create MaterialStackUI for {materialStack.Material.Name}");
        }
    }
    ...
    private void ClearInventoryDisplay()
    {
        ...
        GameLogger.Debug(
            $"AdventurerInventoryUI: ClearInventoryDisplay called. Current child count: {_inventoryList.GetChildCount()}");
        ...
        GameLogger.Debug(
            $"AdventurerInventoryUI: ClearInventoryDisplay completed. New child count: {_inventoryList.GetChildCount()}");
    }
