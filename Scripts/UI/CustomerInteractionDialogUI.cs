...
public partial class CustomerInteractionDialogUI : AcceptDialog
{
    ...
    public void ShowCustomerInteraction(Customer customer, Game.Items.Models.Item? item, ShopManager shopManager)
    {
        ...
        _interactionContext = new ShopInteractionContext
        {
            ...
            AlternativeItemsAvailable =
                shopManager.DisplaySlots.Count(slot => slot != null && slot.CurrentItem != null) > 1,
            TotalInteractions = 0,
    ...
    private void UpdateCustomerDisplay()
    {
        ...
        if (_budget != null)
            _budget.Text =
                $"Budget: {_currentCustomer.BudgetRange.MinSpendingPower}-{_currentCustomer.BudgetRange.MaxSpendingPower}g";
    ...
    private string GenerateEnhancedCustomerThoughts()
    {
        ...
        {
            var decision =
                _customerAI.MakeEnhancedPurchaseDecision(_currentItem, _currentItem.Value, _interactionContext);
            ...
            thoughts.AddRange(decision.Decision switch
            {
                ...
                PurchaseDecision.WantsToNegotiate => new[]
                {
                    $"💰 \"Maybe we can discuss the price? ({decision.NegotiationWillingness:P0} willing to negotiate)\""
                },
    ...
    private void OnOfferDiscountPressed()
    {
        ...
        var response =
            _customerAI.MakeEnhancedPurchaseDecision(_currentItem, _currentItem.Value * 0.9m, _interactionContext);
    ...
    private void OnNegotiatePressed()
    {
        ...
        if (proposedPrice.HasValue)
        {
            ...
            var response =
                _customerAI.MakeEnhancedPurchaseDecision(_currentItem, proposedPrice.Value, _interactionContext);
    ...
    private void OnShowAlternativePressed()
    {
        ...
        if (availableItems.Any())
        {
            ...
            var response =
                _customerAI.MakeEnhancedPurchaseDecision(bestAlternative, bestAlternative.Value, _interactionContext);
            ...
            var alternativeText = response.Decision switch
            {
                ...
                PurchaseDecision.WantsToNegotiate =>
                    $"🤔 \"That {bestAlternative.Name} is interesting. What's your best price?\"",
