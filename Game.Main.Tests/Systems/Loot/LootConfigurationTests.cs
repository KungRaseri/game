...
{
    [Fact]
    public void LootConfiguration_GetMaterialTypes_ReturnsValidMaterials()
    {
        // Act
        var materials = LootConfiguration.GetMaterialTypes();
    ...
    [Fact]
    public void LootConfiguration_GetMaterialTypes_ContainsExpectedMaterials()
    {
        // Act
        var materials = LootConfiguration.GetMaterialTypes();
    ...
    [Fact]
    public void LootConfiguration_GetMaterialTypes_HasCorrectCategories()
    {
        // Act
        var materials = LootConfiguration.GetMaterialTypes();

        // Assert - Verify material categories
        Assert.Equal(MaterialCategory.Metals, materials["iron_ore"].Category);
        Assert.Equal(MaterialCategory.Metals, materials["silver_ore"].Category);
        Assert.Equal(MaterialCategory.Metals, materials["gold_ore"].Category);
        
        Assert.Equal(MaterialCategory.Organic, materials["crude_leather"].Category);
        Assert.Equal(MaterialCategory.Organic, materials["thick_leather"].Category);
        Assert.Equal(MaterialCategory.Organic, materials["herbs"].Category);
        Assert.Equal(MaterialCategory.Organic, materials["coal"].Category);
        
        Assert.Equal(MaterialCategory.Gems, materials["gem_shard"].Category);
        
        Assert.Equal(MaterialCategory.Magical, materials["enchanted_stone"].Category);
        Assert.Equal(MaterialCategory.Magical, materials["mana_crystal"].Category);
    }

    [Fact]
    public void LootConfiguration_GetMaterialTypes_HasCorrectRarities()
    {
        // Act
        var materials = LootConfiguration.GetMaterialTypes();

        // Assert - Verify rarity progression
        Assert.Equal(MaterialRarity.Common, materials["iron_ore"].BaseRarity);
        Assert.Equal(MaterialRarity.Uncommon, materials["silver_ore"].BaseRarity);
        Assert.Equal(MaterialRarity.Rare, materials["gold_ore"].BaseRarity);
        
        Assert.Equal(MaterialRarity.Common, materials["crude_leather"].BaseRarity);
        Assert.Equal(MaterialRarity.Uncommon, materials["thick_leather"].BaseRarity);
        
        Assert.Equal(MaterialRarity.Rare, materials["gem_shard"].BaseRarity);
        Assert.Equal(MaterialRarity.Epic, materials["enchanted_stone"].BaseRarity);
        Assert.Equal(MaterialRarity.Legendary, materials["mana_crystal"].BaseRarity);
    }

    [Fact]
    public void LootConfiguration_GetMaterialTypes_HasReasonableValues()
    {
        // Act
        var materials = LootConfiguration.GetMaterialTypes();
    ...
    public void LootConfiguration_GetLootTables_GoblinHasBasicMaterials()
    {
        ...
        var materials = goblinTable.PossibleDrops.Select(entry => entry.Material.Id).ToList();
        ...
        var ironOreEntries = goblinTable.PossibleDrops.Where(e => e.Material.Id == "iron_ore").ToList();
    ...
    public void LootConfiguration_GetLootTables_TrollHasRareMaterials()
    {
        ...
        var materials = trollTable.PossibleDrops.Select(entry => entry.Material.Id).ToList();
