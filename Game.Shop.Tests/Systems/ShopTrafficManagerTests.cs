...
public class ShopTrafficManagerTests
{
    ...
    public async Task ShopTrafficManager_DuplicateCustomer_RejectsSecondAddition()
    {
        ...
        Assert.True(Enum.IsDefined(typeof(CustomerSatisfaction),
            secondResult)); // Both should return valid satisfaction values
    ...
    public async Task ShopTrafficManager_SessionDuration_TracksAccurately()
    {
        ...
        Assert.True(trafficRecord.SessionDuration <=
                    endTime - startTime + TimeSpan.FromSeconds(1)); // Allow for timing precision
    }
